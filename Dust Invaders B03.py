...
#
# Version B.0.3 - Control version to check engagement allure  
#
...
def save_data(time2save, token2save, r2save, l2save, u2save, d2save) :
    time2save_string = str(time2save[3]) + "-" + str(time2save[4]) + "-" + str(time2save[5]) + "\t" + str(time2save[0]) + ":" + str(time2save[1]) + ":" + str(time2save[2]) + "\t" + str(time2save[6])ave
    string2save = time2save_string + "\t" + str(token2save) + "\t" + str(r2save) + "\t" + str(l2save) + "\t" + str(u2save) + "\t" + str(d2save) + "\n"
    try :  
        with open('vacuum_data_smile.txt', 'r') as my_file:
            saved_data = my_file.read()
    except OSError : 
        saved_data = "" 
        with open('vacuum_data_smile.txt', 'w') as my_file:
            my_file.write("")
